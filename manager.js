...
function step(st) {
  ...
  if (st.gamesSinceRefresh >= REFRESH_GAMES) {
    try { execSync('node replays.js mine', {cwd: __dirname, stdio: 'ignore'}); execSync('node archetypes.js build', {cwd: __dirname, stdio: 'ignore'}); console.log(new Date().toISOString(), 'opponent model + team library refreshed'); } catch (e) { console.log('refresh failed', e.message); }
    ...
    // ---- crawl a slice of the public archive (bounded), then datasets, then the networks (each saves only if it beats the incumbent)
    if (process.env.CRAWL !== '0') { try { execSync(`timeout ${process.env.CRAWL_MINUTES || 20}m node crawl.js data/formats.json 4 250`, {cwd: __dirname, stdio: 'ignore'}); console.log(new Date().toISOString(), 'crawl slice done'); } catch (e) { console.log(new Date().toISOString(), 'crawl slice ended (time cap or error)'); } }
    try { execSync('node dataset.js moves', {cwd: __dirname, stdio: 'ignore', timeout: 30 * 60 * 1000}); const p = execSync('python3 train.py moves data/moves.jsonl', {cwd: __dirname, timeout: 60 * 60 * 1000}).toString().trim(); console.log(new Date().toISOString(), 'policy net:', p.split('\n').filter(l => /FINAL|saved|not saved/.test(l)).join(' | ').slice(0, 200)); } catch (e) { console.log('policy net training failed', e.message.slice(0, 80)); }
    try { execSync('node dataset.js value', {cwd: __dirname, stdio: 'ignore', timeout: 30 * 60 * 1000, env: process.env}); const p = execSync('python3 train.py value data/value.jsonl', {cwd: __dirname, timeout: 30 * 60 * 1000}).toString().trim(); console.log(new Date().toISOString(), 'value net:', p.split('\n').filter(l => /FINAL|saved|not saved/.test(l)).join(' | ').slice(0, 200)); } catch (e) { console.log('value net training failed', e.message.slice(0, 80)); }
    try { const p = execSync('node predict2.js train', {cwd: __dirname, timeout: 15 * 60 * 1000}).toString().trim(); console.log(new Date().toISOString(), 'linear predictor:', p.split('\n')[0].slice(0, 140)); } catch (e) { console.log('predictor training failed', e.message.slice(0, 80)); }
