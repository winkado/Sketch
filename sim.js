...
function ourChoice(req, st, opts) {
  ...
  req.active.forEach((act, i) => {
    ...
    if (me.species === 'Oranguru') {
      ...
      else if (partner && !['Sinistcha', 'Farigiraf'].includes(partner.species) && mv('Helping Hand') && st.tr && st.active.p2.some(f => f && ((f.revealed && ([...f.revealed].some(m => /Rage Powder|Follow Me/.test(m)))) || (/Sinistcha|Clefable|Indeedee|Maushold/.test(f.species) && !(f.revealed && f.revealed.size >= 3))))) c = 'move Helping Hand ' + allyTarget(slotIdx >= 0 ? slotIdx : i);   // a redirector is up: Instruct would be pulled
      else if (partner && !['Sinistcha', 'Farigiraf'].includes(partner.species) && mv('Instruct') && st.tr && st.active.p2.some(f => f && ((f.revealed && ([...f.revealed].some(m => /Rage Powder|Follow Me/.test(m)))) || (/Sinistcha|Clefable|Indeedee|Maushold/.test(f.species) && !(f.revealed && f.revealed.size >= 3)))) && mv('Protect') && !me.protectedLast) c = 'move Protect';   // redirector up, no Helping Hand: don't feed them the Instruct
